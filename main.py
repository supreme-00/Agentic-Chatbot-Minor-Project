        ...
        if detected_type == QueryType.GENERAL:
            try:
                response = client.models.generate_content(
                    ...
                    contents=f"You are Ganpat University's friendly assistant. Answer briefly: {user_message}",
                    config=types.GenerateContentConfig(temperature=0.7, max_output_tokens=512)
                )
                if response and response.text:
                    return {"reply": response.text.strip()}
                return {"reply": "I'm not sure how to help with that. Try asking about students, teachers, or timetables."}
            except:
                return {"reply": "I'm having trouble responding. Please try again."}
